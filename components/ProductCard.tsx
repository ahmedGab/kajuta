    ...
    <div className="card-premium group flex flex-col h-full bg-white relative">
      <Link href={`/produits/${product.slug}`} className="block relative aspect-square overflow-hidden bg-mint/20">
        <Image
          ...
          alt={product.alt || product.name || "Produit Cajuta"}
          fill
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className={`flex items-center justify-between mt-auto ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <a
            href={`https://wa.me/21650123456?text=${encodeURIComponent(isRTL ? `مرحبا، أريد طلب ${product.name}` : `Bonjour Cajuta, je veux commander: ${product.name}`)}`}
            target="_blank"
            ...
            aria-label={isRTL ? `طلب ${product.name}` : `Commander ${product.name}`}
          >
